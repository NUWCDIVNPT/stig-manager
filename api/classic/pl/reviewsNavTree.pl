#!/usr/bin/perl
# $Id: reviewsNavTree.pl 885 2018-02-20 16:26:08Z bmassey $
...
END
	...
	foreach $packageName (sort keys %$packages) {
		...
		if ($userObj->{'role'} eq 'Staff') {
			$nessusLeaf = {};
			$nessusLeaf->{'text'} = 'Scan Management...';
			$nessusLeaf->{'action'} = 'nessus';
			$nessusLeaf->{'packageId'} = $packageId;
			$nessusLeaf->{'packageName'} = $packageName;
			$nessusLeaf->{'id'} = $packageId . "-nessus-node";
			$nessusLeaf->{'iconCls'} = 'sm-nessus-16-icon';
			$nessusLeaf->{'leaf'} = 'true';
			push(@$packageArray,$nessusLeaf);
		}
