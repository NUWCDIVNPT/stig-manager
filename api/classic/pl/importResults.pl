#!/usr/bin/perl
#$Id: importResults.pl 891 2018-05-21 22:25:10Z cdaley $
...
# or use this code block to avoid unecessary instantation above, in the case of ckl v2...:
# my $cklContent = read_file($blobName);
# my $cklVersion = getCklVersion($cklContent);
# if ($extension eq 'ckl') {
	# $cklContent = read_file($blobName);
	# $cklVersion = getCklVersion($cklContent);
	# if ($cklVersion == 2){
		# my $xml = new XML::Twig;
	# }
	# elsif ($cklVersion == 1){
...
if ($source eq 'review') {
	...
	if ($extension eq 'ckl') {
		...
		# Extension = 'ckl'
		###################
		# my ($stigTitle) = $dbh->selectrow_array("select title from stigs.stigs s where s.stigId = ?",undef,($cgiStigId)); #only necessary for cklv1...could be avoided for ckl v2
		# my $cklContent = read_file($blobName);
