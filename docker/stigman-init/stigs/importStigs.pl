...
my $host = "db";
my $port = "1521";
my $db = "orclpdb1.localdomain";
my $connStr = "DBI:Oracle:$host:$port/$db";
my $dbh = DBIx::Log4perl->connect($connStr,"stigs","stigs");
...
sub importStig{
	...
	print "INSERTING " . scalar @{ $groupArrayRef } . " GROUPS...\n";
	...
	print "INSERTED GROUPS FOR: $revisionId\n";
	#$dbh->commit;
...
sub processProfiles {
	...
	print "INSERTING " . scalar @{$revIds} . " PROFILE TUPLES...\n";
	my $tuples = $sthRPGMap->execute_array({ArrayTupleStatus=>\my @tuple_status});
	if (defined $tuples) {
		#print "REV-PROFILE-GROUP TUPLES: $tuples\n";
	} else {
