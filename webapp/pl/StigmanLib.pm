...
#########################################
# Example of the hash that is returned:
#########################################
# $userRef = {
#          'roles' => {
#                       'Administrator' => {
...
sub getUserObject {
	my $SESSION_TIMEOUT = 600;
	#my $SESSION_TIMEOUT = 99999;
	my ($now,$userRef,$ourScript,$key,$paramsStr,$redirectUrl,$sql,$labArray);
	my @params;
	my %cgiParams;
...
SELECT
	...
	sid.lastUse as "lastUse"
FROM 
	stigman.stigman_ids sid
	left join stigman.user_data ud on UPPER(sid.cn)=UPPER(ud.cn)
	left join stigman.roles r on r.id=ud.roleId
where
	sid.guid=?
END
	$userRef = $dbh->selectrow_hashref($sql,undef,($guid));
	if ($now - $userRef->{'lastUse'} > $SESSION_TIMEOUT && !$noredirect) {
		# Get the name of the script that called us
		($ourScript) = ($0 =~ /.*[\\\/](.*)/);
		push(@params,"redirect=$ourScript");
		%cgiParams = $cgi->Vars;
		foreach $key (keys %cgiParams){
			push(@params,"$key=" . uri_escape($cgiParams{$key}));
		}
		$paramsStr = join("&",@params);
		$redirectUrl = "getUserObject.pl?" . $paramsStr;		
