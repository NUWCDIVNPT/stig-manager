...
function getAccessToken({redirectUri}) {
  if (!tokens.accessToken) {
    console.log(logPrefix, 'getAccessToken, redirecting to authorization')
    return createAuthorization(redirectUri)
  }
  return {
...
async function broadcastNoToken() {
  ...
  const redirectUrl = `${url.origin}${url.pathname}reauth.html`
  const auth = await createAuthorization(redirectUrl)
