...
Click the "Record" button to start recording the log stream to a file. The "Record" function is not available in Firefox.
The "Preserve Log" checkbox will retain log entries in the panel when the stream is stopped and then started again. 
"Wrap" will toggle line wrapping in the panel. The "Clear" button will clear all log entries from the panel.
