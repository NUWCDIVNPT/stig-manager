...
.. note::
   "Latest" is determined strictly by the STIG's version and release numbers, regardless of the ``status`` value DISA assigns to a revision (``accepted``, ``deprecated``, ``draft``). DISA sometimes publishes revisions with new rule content but a ``deprecated`` status; STIG Manager will treat such a revision as the latest if its version+release is higher than any other imported revision of the same STIG. To override this behavior for a particular Collection, :ref:`pin <pin-revision>` the desired Revision in Collection Management.
