...
Conversely, when STIG Manager produces a .ckl file from an Asset that has the below metadata values set, it will populate the appropriate .ckl elements. When exporting, the ``cklWebOrDatabase`` metadata value alone determines the ``<WEB_OR_DATABASE>`` element: a case-insensitive ``true`` exports as ``true``; any other value, or an absent key, exports as ``false``. ``cklHostName`` replaces the Asset name in the ``<HOST_NAME>`` element only when the Asset exports as Web or Database.
