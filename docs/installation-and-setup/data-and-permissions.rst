...
Conversely, when STIG Manager produces a .ckl file from an Asset that has the below metadata values set, it will populate the appropriate .ckl elements. When exporting, an explicit ``cklWebOrDatabase`` metadata value determines the ``<WEB_OR_DATABASE>`` element (only a case-insensitive ``true`` exports as ``true``); if that key is absent, the presence of ``cklHostName`` metadata marks the Asset as Web or Database. ``cklHostName`` replaces the Asset name in the ``<HOST_NAME>`` element only when the Asset exports as Web or Database.
