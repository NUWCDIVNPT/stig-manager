...
function initialize(options) {
  ...
  return new Promise((resolve) => {
    ...
    timeoutId = setTimeout(() => {
      ...
      stateWorkerChannel.postMessage({
        type: 'state-error',
        data: 'Timeout waiting for first SSE message',
      })
      resolve({
        success: false,
        error: `Timeout waiting for API state event stream.<br><br>
...
async function getApiState() {
  ...
  if (!response.ok) {
    return {
      success: false,
      error: `Failed to get API state: ${response.status} ${response.statusText}`,
    }
  }
...
function reconnectSSE() {
  ...
  eventSource.onerror = function (error) {
    stateWorkerChannel.postMessage({
      type: 'state-error',
      data: { message: `Cannot connect to ${error.target.url}` },
    })
