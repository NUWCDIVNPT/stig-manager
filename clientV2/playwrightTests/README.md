...
- ✅ **Real JWT tokens** - Tokens are generated and signed by the local MockOidc server

## 🚦 Prerequisites

**You must have BOTH of these running before you start tests:**

1. **Your API** (e.g. http://localhost:64001)
2. **MockOidc server** (local OIDC mock, see `tests/utils/mockOidc.js`)
   - Install dependencies: `npm install jsonwebtoken ms`
   - Start with: `node tests/utils/mockOidc.js`
   - Default port: 8080
...
```bash
cd /path/to/clientV2/tests/utils
npm install jsonwebtoken ms  # Only needed once
node mockOidc.js             # Starts on http://localhost:8080
```
...
```
E2E Test
  ├─> Mocked OIDC Worker (browser-side OAuth flow is mocked)
  ├─> Real JWT tokens from MockOidc (RS256 signed)
  ├─> Real State Worker (API health monitoring via SSE)
  └─> Real API Calls
      └─> API receives real JWT
      └─> API validates token structure
      └─> API returns real data from database
```

Your API receives **real JWT tokens** generated by MockOidc, not just mock payloads. The API extracts user info (roles, permissions) from the token and enforces real authorization.
...
**401 Errors?**
- API is rejecting tokens
- Configure API to skip signature verification in test mode
- Make sure MockOidc is running and reachable

**Tests time out?**
- Check if API is running: `curl http://localhost:64001/api/collections`
- Check if MockOidc is running: `curl http://localhost:8080/.well-known/openid-configuration`
...
| Fast tests? | ✅ Yes (~2-3s) |
| CI/CD friendly? | ✅ Yes (just needs API & MockOidc) |
