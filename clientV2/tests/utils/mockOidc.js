...
class MockOidc {
  ...
  getToken({
    ...
    if (issuedAt) {
      payload.iat = Number.parseInt(issuedAt)
    }
    if (sid) {
      payload.sid = sid
    }
...
if (fileURLToPath(import.meta.url) === path.resolve(process.argv[1])) {
  ...
  for (const [index, arg] of args.entries()) {
    if (arg === '--port' || arg === '-p') {
      ...
      options.keyCount = Number.parseInt(args[index + 1], 10)
    }
